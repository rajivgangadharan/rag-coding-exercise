...
from cohere.types.generation import Generation
import gradio as gr
...
# importing prompt for llm interaction
...
class VexStor:
    ...
    def ensure_collection_exists(self, collection_name):
        try:
            ...
            try:
                # automatically getting the dimensions from the embedder
                # it should be 1024 in this case
    ...
    def similarity_search(
        ...
        **kwargs,
        ...
        search_results = self.vectorstore.similarity_search(
            query=query, k=top_k, collection=collection, **kwargs
        )
        #
...
class LanguageModel:
    ...
    def __init__(self, model, vexstor: VexStor, **kwargs):
        if model is None:
            self._model = os.getenv("LLM_MODEL_ID", "command-r")
            if model is None:
                raise Exception(
                    f"Cannot determine model, set LLM_MODEl_ID in the .env file"
                )
        _api_key = os.getenv("COHERE_API_KEY")
        if _api_key is None:
            raise Exception(
                f"API key is unavailable for authentication, set COHERE_API_KEY in .env file"
            )

        self._max_tokens = kwargs.get("max_tokens", 300)
        self._temperature = kwargs.get("temperature", 0.3)
        self._model = model

        self.llm = cohere.Client(_api_key)

    def __call__(self, context, query, prompt):
        final_prompt = prompt.format(context=context, query=query)
        logger.debug(f"Final Prompt for LLM is {final_prompt}")
...
def main():
    ...
    results = vs.qdrant.search(query=query, search_type="similarity", k=1)
    logger.debug("| printing the vector store fetch ")
    for r in results:
        logger.debug(f"| {r}")
    logger.debug("| vector store fetch end. ")
    results = results[0].page_content
    logger.debug(f"Context is {results}\n")

    model_id = os.getenv("LLM_MODEL_ID", "command-r-plus")
    lm = LanguageModel(model=model_id, vexstor=vs)
    response: Generation = lm(context=results, query=query, prompt=prompt)
    for i, r in enumerate(response.generations):
        print(f"RESPONSE: {i} {r}\n")

    print(f"TEXT = {response.generations[0].text}")
